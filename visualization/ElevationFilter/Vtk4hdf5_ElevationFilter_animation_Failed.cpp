...
*/
  ...
  vtkGenericMovieWriter * writer;
    /* Not work
      ...
      vtkS.martPointer<vtkAVIWriter> writer=vtkSmartPointer<vtkAVIWriter>::New();
      */
  std::string name_to_write(filename);
  name_to_write.append(".avi");
  // char *  name_to_write = vtkGenericMovieWriter::FileName
  // name_to_write = filename.append(".avi");

  writer -> vtkSetStringMacro(name_to_write.c_str());
  // writer -> Start();
  // writer -> End();
  // writer -> Write();
